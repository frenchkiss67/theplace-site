...
import com.theplace.receiptscanner.util.openPdf
import com.theplace.receiptscanner.util.sharePdf
...
fun ReceiptListScreen(
    ...
    val storage = remember { PdfStorage(context) }
            ...
            Column(modifier = Modifier.padding(padding).fillMaxSize()) {
                ...
                LazyColumn(
                    ...
                    items(receipts, key = { it.id }) { receipt ->
                        ReceiptCard(
                            receipt = receipt,
                            onOpen = { openPdf(context, storage.shareUri(receipt.fileName)) },
                            onShare = {
                                sharePdf(
                                    context,
                                    storage.shareUri(receipt.fileName),
                                    receipt.name,
                                )
                            },
        ...
        Column(modifier = Modifier.padding(16.dp)) {
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                Column(modifier = Modifier.weight(1f)) {
                    ...
                    Text(
                        text = "${stringResource(
                            R.string.pages_label,
                            receipt.pageCount,
                        )}  ·  ${formatSize(context, receipt.sizeBytes)}",
                        style = MaterialTheme.typography.bodySmall,
                        color = MaterialTheme.colorScheme.onSurfaceVariant,
                    )
                }
